...
- `AUTH-08` 四类角色使用同一登录入口但获得 14/11/7/7 个差异化服务端导航，首页分别为 `/admin/home`、`/community/home`、`/grid/home`、`/resident/home`。网格员的底层范围只读权限不形成通用管理菜单；直接访问未授权业务页进入无权页。
...
- `WORKBENCH-01` 四个角色首页分别聚合全局风险、社区待办、本人执行队列和本人服务进度；网格员统计按执行人过滤，居民统计按本人过滤。
- `WORKBENCH-02` 管理员、社区、网格员和居民的动态入口精确为 14/11/7/7，每个入口必须映射真实 API、权限和数据库数据。
...
当前 Flyway 迁移链形成 23 张业务表，按模块划分如下：
...
| 管理运行 | `/api/system/operations`、`/api/system/health` | 聚合审计和系统一致性健康 |
