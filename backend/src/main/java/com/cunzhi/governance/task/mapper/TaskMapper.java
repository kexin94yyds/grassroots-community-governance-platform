...
public interface TaskMapper {
    ...
    @Select("""
            ...
            join sys_user dispatcher on dispatcher.id = t.dispatcher_user_id
            join sys_user assignee on assignee.id = t.assignee_user_id
            where t.patrol_plan_id = #{patrolPlanId}
            for update
            """)
    Optional<TaskRow> findByPatrolPlanIdForUpdate(@Param("patrolPlanId") long patrolPlanId);
    ...
    @Select("select count(*) from work_task where id = #{taskId} and patrol_plan_id is not null")
    int countPatrolTaskById(@Param("taskId") long taskId);
    ...
    @Insert("""
            insert into work_task
              (task_no, source_event_id, patrol_plan_id, grid_id, task_type, title, description, priority,
               status, dispatcher_user_id, assignee_user_id, due_at)
            values
              (#{taskNo}, null, #{patrolPlanId}, #{gridId}, 'ROUTINE_INSPECTION', #{title}, #{description}, #{priority},
               'PENDING_ACCEPT', #{dispatcherUserId}, #{assigneeUserId}, #{dueAt})
            """)
    int insertPatrolTask(
            @Param("taskNo") String taskNo,
            @Param("patrolPlanId") long patrolPlanId,
            @Param("gridId") long gridId,
            @Param("title") String title,
            @Param("description") String description,
            @Param("priority") String priority,
    ...
    @Update("""
            update patrol_plan plan
            join work_task task on task.patrol_plan_id = plan.id
            set plan.status = 'COMPLETED', plan.completed_at = current_timestamp(3), plan.version = plan.version + 1
            where task.id = #{taskId} and plan.status = 'ACTIVE'
            """)
    int completePatrolPlanByTaskId(@Param("taskId") long taskId);

    @Update("""
            update patrol_plan plan
            join work_task task on task.patrol_plan_id = plan.id
            set plan.status = 'CANCELLED', plan.cancelled_at = current_timestamp(3), plan.version = plan.version + 1
            where task.id = #{taskId} and plan.status = 'ACTIVE'
            """)
    int cancelPatrolPlanByTaskId(@Param("taskId") long taskId);
